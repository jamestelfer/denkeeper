...
### No live bot in any phase

**Every phase in this plan is verifiable from `go test` alone. No phase requires a Telegram bot
token, a real chat, or a look at a physical device.** The Phase 0 seam exists precisely so that
outbound bytes can be asserted in-process, and every later phase's verification asserts on captured
`tgbotapi.MessageConfig` values or on golden renderer output.

This has a consequence worth stating plainly: **these tests prove what we send, not what Telegram
displays.** Assertions about rendered appearance — whether an indent character survives, whether
`pre` is monospaced on a phone, whether Telegram autolinks a bare scheme, whether two consecutive
`pre` blocks read as one broken block — are outside what any test here can establish. Where a phase
previously leaned on a device check for that class of question, the plan now:

- decides the behaviour from Telegram's published API documentation,
- pins the resulting bytes with a test so the decision cannot silently regress, and
- records the appearance question as one for **system testing, which is a separate exercise tracked
  outside this plan**.

Broken input is still verifiable without a live bot: a characterization test asserts that today's
mangled markdown is forwarded verbatim, which is exactly the behaviour Phase 1 inverts. Where a
question genuinely cannot be closed from the documentation, treat it as an open question carried into
system testing — not as a reason to reach for a token.
...
- [ ] `[observable]` A characterization test asserts the bug-report input
      (`See https://example.com/a_b and https://example.com/c_d`) reaches the bot byte-identical with
      `ParseMode == "Markdown"` — the broken-markdown-is-forwarded fact that Phase 1 inverts
...
today's Markdown behaviour, not tomorrow's HTML behaviour. Run `just hook` for the full gate.

Then confirm the no-op claim in-process, which is what the seam is for. A characterization test feeds
the adapter markdown that today's `parse_mode=Markdown` path mangles — the bug report's
`See https://example.com/a_b and https://example.com/c_d` — and asserts the captured
`MessageConfig.Text` is **byte-identical to the input**, with `ParseMode == "Markdown"`. That is the
current defect pinned as a fact: the adapter forwards the broken markdown untouched and lets Telegram
mangle it. Phase 1 inverts exactly this assertion, so the diff between the two phases is the
behaviour change, visible in the test file.

Nothing in this phase needs a bot token. A Phase 0 that changes outbound bytes has failed, and the
captured-bytes assertions plus `git diff` are what establish that it did not.
...
`just hook`, then `just scan` for the new dependency.

The originating defect is verified in-process, not against a live bot. The Phase 0 characterization
test that asserted the bug-report input goes out as raw markdown is **rewritten** here to assert the
rendered HTML: both URLs byte-identical, both wrapped in anchors, `ParseMode == "HTML"`. A second case
covers a literal `snake_case_identifier` in prose surviving with every underscore and no backslash.
That inversion — same input, same assertion site, changed expectation — is the phase's proof.

For the activity log, add an adapter test that sends the dispatcher's actual shape:
`OutgoingMessage{ParseMode: "HTML", Text: "<blockquote expandable>…</blockquote>", ReplyMarkup: <inline keyboard>}`,
and assert the captured text is byte-identical and the markup is unchanged. This is the highest-blast-radius
regression in the phase and it is fully assertable in-process — do not defer it to a manual approval flow.
...
- Telegram collapses some leading whitespace in HTML mode, so the indent character for R9 is a real
  decision. **Resolve it from Telegram's published HTML-subset documentation, not from a live chat**,
  pick the character it supports, and write the choice and its source into the code as a comment. Pin
  the chosen bytes with a golden test so the decision cannot silently regress. Whether the indent is
  *visible* on a given client is a system-testing question — record it as one and move on. If the
  documentation does not settle it, note the ambiguity in "Lessons learned", ship the
  non-breaking-space option (the more likely survivor of whitespace collapsing), and hand the
  appearance check to system testing.
- `blockquote` nesting rules in Telegram's subset may forbid a quote inside a list item. Check
  Telegram's documented tag list for whether nesting is permitted; if it is not documented either
  way, choose a degradation behaviour, assert the chosen bytes in a golden test, and record the
  uncertainty for system testing.
...
- [ ] `[observable]` Golden test: one kitchen-sink document containing all six constructs together
      (wrapped prose, hard break, thematic break, block quotation, nested bullet list, ordered list)
      renders with every construct correct in a single expected output — the constructs must not
      interfere with each other
- [ ] `[observable]` Adapter test: the kitchen-sink document sent through the adapter reaches the bot
      with `ParseMode == "HTML"` and no fallback triggered
- [ ] `[structural]` All blocks remain tag-balanced under the Phase 1 helper (R5 still holds)

### Verification

Run the golden suite, then `just hook`. The R6/R34 check that matters is the realistic one: render a
multi-paragraph LLM-style reply with prose wrapped at 80 columns and assert no two words are fused
across any source line break — this is the defect class most likely to survive a narrow per-construct
test.

For R9, assert the indent bytes chosen above appear at the start of each nested item and that nesting
depth is recoverable from the output. Whether that indent is *visible* to a user is not assertable
here and belongs to system testing — the plan's job is that the bytes are correct and pinned.
...
- Telegram's documentation shows no indent character that survives whitespace collapsing, leaving R9
  unimplementable from the byte level alone
...
**Carry-forward**: Re-verify Phases 1–2 — the full golden suite, including the wrapped-prose
no-fused-words case.
...
- [ ] `[observable]` Golden test: a fenced code block containing `<script>` renders escaped, not as
      markup
- [ ] `[observable]` Golden test: for a rejected scheme, the **destination string does not appear
      anywhere in the output** — not in an `href`, not as text. Assert its absence, not just the
      absence of `<a`
- [ ] `[observable]` `just scan` passes with no new gosec finding on the URL-handling code

### Verification

Golden suite plus `just hook` plus `just scan`.

The autolink concern is closed by construction rather than by inspection. Telegram autolinks
plain text *after* parsing, so dropping the anchor while leaving `javascript:alert(1)` in the visible
text would hand Telegram a string it might turn back into a link. R16 emits the label only and drops
the destination, so the deterministic assertion is that the destination substring is absent from the
rendered output entirely. A string that was never sent cannot be autolinked — this replaces the device
check with a stronger guarantee, not a weaker one.

Monospacing of code fences is an appearance question: assert the `<pre>`/`<code>` structure and the
language annotation in the golden file, and leave rendering fidelity to system testing.

### Regression watchpoints

- Telegram autolinking plain text. Dropping an anchor does not by itself guarantee the destination
  stays inert — the destination must not reach the output at all. Assert its absence in the golden
  file; that is what makes the autolink path unreachable.
...
- Fixed-width alignment assumes monospaced rendering, which assumes `pre`. Telegram documents `pre`
  as preformatted monospace; take that as the basis for the column algorithm and note that per-client
  fidelity is a system-testing question. The algorithm's own correctness — equal column offsets on
  every row — is fully assertable on the output string, which is what this phase pins.
...
- [ ] `[observable]` Golden test: a realistic table with widely varying cell widths produces identical
      column start offsets on every row, asserted by computing the offsets from the output rather than
      by eyeballing an expected string
- [ ] `[observable]` Adapter test: a table-bearing reply reaches the bot with `ParseMode == "HTML"`
      and triggers no fallback — no plain-text retry, no empty parse mode
- [ ] `[structural]` Both constructs emit tag-balanced blocks (R5 still holds)

### Verification

Golden suite and `just hook`. Alignment is verified arithmetically: parse the rendered rows, compute
each column's start offset, and assert equality across rows. That is a stronger check than looking at
a phone, because it fails on the one-character error a human eye forgives.

This phase's whole value is that it does not fall back, so assert that directly — the adapter test
above must show a single HTML send with no retry. Once Phase 7 lands, extend it to assert the fallback
counter did not move.

Whether the aligned columns *look* right on a narrow screen is a system-testing question. Note it and
move on; a table that is arithmetically aligned and still ugly is a design issue, not a defect in this
phase.
...
- System testing later shows `pre` is not monospaced consistently across clients, invalidating the
  alignment approach — a replan trigger for a follow-up change, not a blocker for this phase
...
- Rate limiting. Sending several messages in immediate succession may hit Telegram's per-chat flow
  limits, surfacing as a 429 mid-sequence. R31 says abort — decide whether that is the desired
  experience for a rate limit specifically, or whether a bounded backoff belongs here. This is a
  **design decision, not a measurement**: take Telegram's documented per-chat limits as the input,
  decide, record the choice in "Lessons learned", and cover the behaviour by injecting a 429-shaped
  error at chunk *n* through the Phase 0 fake. Whether real-world traffic actually trips the limit is
  a system-testing question.
...
- [ ] `[observable]` Reassembly test: a >4096-character reply of 60 numbered lines with mixed
      formatting is sent through the adapter; concatenating the captured sends in call order yields
      every number exactly once, in ascending order, with no duplication and no loss

### Verification

Run the chunker and adapter suites, then `just hook`.

The reassembly test is the one that matters, and it replaces the device check with something more
reliable than reading messages on a phone. The Phase 0 fake records every outbound `MessageConfig` in
call order, so the test concatenates their `Text` fields and asserts against the source: all 60
ordinals present, each exactly once, in order. Ordering and duplication bugs — the two failure modes
a human reading a long thread is worst at spotting — become a hard assertion. Also assert every
captured chunk is within the limit and tag-balanced.

For buttons, assert on the captured `ReplyMarkup`: non-nil on the final send, nil on all others. No
approval flow needs to be triggered by hand to establish that.
...
**Carry-forward**: Re-verify Phase 5 in full — every chunker test and the long-reply reassembly test.
...
- Telegram may render two consecutive `pre` blocks with visible separation that reads as a broken
  code block. This is purely an appearance question and cannot be settled here. Decide the
  continuation-marker policy now (a marker, or none), record the decision and its reasoning in
  "Lessons learned", pin the resulting bytes with a test, and flag the appearance for system testing.
  Prefer no marker as the default — a marker that turns out to be unnecessary is content the user did
  not write, added to every split block.
...
- [ ] `[observable]` Reassembly test: a ~10,000-character fenced code block sent through the adapter
      produces several sends, each a well-formed `pre`/`code` block within the limit, and the
      concatenated text content equals the original source exactly — asserted by string comparison,
      not by inspection

### Verification

Chunker suite and `just hook`.

Use a real input, not a synthetic one: embed an actual Go source file as a testdata fixture, render it
as a fenced block, send it through the fake, strip the tags from each captured chunk, concatenate, and
assert equality with the fixture. Byte equality against the source file is the no-loss proof the plan
asks for, and it is exactly the check the plan previously described as "diff the reassembled received
text" — done in the test suite instead of by hand, which is where it belonged.

Also assert no chunk contains a fragment of a tag, by running each chunk through the Phase 1
tag-balance helper.
...
- System testing later shows split code blocks render unacceptably, requiring a different strategy
  such as attaching oversized code as a document — a follow-up change, not a blocker for this phase
...
- [ ] `[observable]` A test wires the adapter to a real OTel SDK meter provider backed by
      `metric.NewManualReader()`, triggers a fallback, collects, and asserts the counter is present
      with the expected reason attribute and a value of 1 — proving the instrument is registered and
      recording, not a silent no-op
...
Run the adapter suite and `just hook`.

The "a silently no-op meter passes unit tests" risk is real and is why the manual-reader test above
uses the **real OTel SDK** rather than a hand-rolled fake meter. A `ManualReader` collects from a
genuine `MeterProvider`, so an unregistered or misnamed instrument produces an empty collection and
the test fails — the same failure a `curl` on `/metrics` would have caught, without a running
instance or a bot token.

Assert the three cases against one collection each: render failure increments with the render reason,
injected HTML rejection increments with the rejection reason, successful send collects no data point
for the counter. For the log side, capture `slog` output with a `slog.Handler` writing to a buffer and
assert exactly one record at `LevelWarn` naming the reason.

Whether the counter is scrapable through the deployed Prometheus endpoint is deployment wiring, not
this phase's behaviour — the existing exporter carries it, and confirming the scrape belongs to system
testing.

### Regression watchpoints

- Log volume. A reason that fires on ordinary content would flood warnings. Assert the quiet-path case
  over the Phase 2 kitchen-sink document and the Phase 5 long-reply fixture: if the counter moves on
  either, that is a Phase 1–6 defect this phase has just made visible — investigate it rather than
